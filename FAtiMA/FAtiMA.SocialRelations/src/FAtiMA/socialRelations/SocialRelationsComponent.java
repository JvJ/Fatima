...
public class SocialRelationsComponent implements Serializable,IAppraisalDerivationComponent, IModelOfOtherComponent,IProcessEmotionComponent {
	...
	{
		if (e.GetSubject().equals(Constants.SELF_STRING) && e.GetAction().equals("look-at")) 
		{					
			...
			int relationShip = Math.round(LikeRelation.getRelation(Constants.SELF_STRING, target).getValue(am.getMemory()));
			if (relationShip != 0) {
	...
	public void emotionActivation(AgentModel am, ActiveEmotion em) {
		...
		if (em.getType().equalsIgnoreCase(OCCEmotionType.ADMIRATION.name())) 
		{
			...
			{
				LikeRelation.getRelation(Constants.SELF_STRING, em.GetDirection().toString()).increment(m, em.GetIntensity());
				RespectRelation.getRelation(Constants.SELF_STRING, em.GetDirection().toString()).increment(m,em.GetIntensity());
			}
		} else if (em.getType().equalsIgnoreCase(OCCEmotionType.REPROACH.name())) 
		{
			if (em.GetDirection() != null) 
			{
				LikeRelation.getRelation(Constants.SELF_STRING, em.GetDirection().toString()).decrement(m,em.GetIntensity());
				RespectRelation.getRelation(Constants.SELF_STRING, em.GetDirection().toString()).decrement(m,em.GetIntensity());
			}
		...
		(OCCEmotionType.GLOATING.name())) {
			...
			if (em.GetDirection() != null) {
				LikeRelation.getRelation(Constants.SELF_STRING, em.GetDirection().toString()).decrement(m,

				em.GetIntensity());
			}
		} else if (em.getType().equalsIgnoreCase(OCCEmotionType.JOY.name())) {
			if (em.GetCause().GetTarget() != null && em.GetCause().GetTarget().equals(Constants.SELF_STRING)) {
				LikeRelation.getRelation(Constants.SELF_STRING, em.GetCause().GetSubject()).increment(m,em.GetIntensity());
			}
		} else if (em.getType().equalsIgnoreCase(OCCEmotionType.DISTRESS.name())) {
			if (em.GetCause().GetTarget() != null && em.GetCause().GetTarget().equals(Constants.SELF_STRING)) {
				LikeRelation.getRelation(Constants.SELF_STRING, em.GetCause().GetSubject()).decrement(m,
	...
	public void inverseAppraisal(AgentModel am, AppraisalFrame af) {
		...
		if (like != 0) {
			e = af.getEvent();
			LikeRelation.getRelation(Constants.SELF_STRING,e.GetTarget()).setValue(am.getMemory(), like);
		}
	}
	...
	public void parseAdditionalFiles(AgentModel am) throws Exception 
	{
		...
		for (String file : _parsingFiles) {
			if(file != null)
			{
				SAXparser.parse(new File(file),new RelationsLoaderHandler(am));
			}
		}
	}
