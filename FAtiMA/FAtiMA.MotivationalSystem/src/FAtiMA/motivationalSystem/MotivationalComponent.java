...
public class MotivationalComponent implements Cloneable,
	...
	public void appraisal(AgentModel am, Event e, AppraisalFrame as) {
		...
		if(e.GetSubject().equals(Constants.SELF_STRING) || (e.GetTarget() != null && e.GetTarget().equals(Constants.SELF_STRING)))
		{
	...
	public float getContributionToNeeds(AgentModel am, ActivePursuitGoal g) {
		...
		for (EffectOnDrive e : effects.getEffects()) {
			...
			if (am.isSelf()) {
				test = groundedEffect.getTarget().toString().equals(Constants.SELF_STRING);
			} else {
	...
	public void update(AgentModel am, Event e) {
		...
		if(result != 0)
		{
			if(_appraisals.containsKey(e.toString()))
			{
				oldValue = _appraisals.get(e.toString());
			}
			else
			{
				oldValue = 0;
			}
			_appraisals.put(e.toString(), oldValue + result);
		}
	}
		...
		for (ActionEffectsOnDrives actionEffects : _actionEffectsOnDrives.values()) {
			...
			if (Unifier.PartialUnify(e.toStepName(), actionName, substitutions)) {
				...
				for (EffectOnDrive eff : actionEffects.getEffects()) {
					...
					if (target.toString().equals(Constants.SELF_STRING)) {
	...
	{
		Event e;
		float contributionToNeeds = 0;
		Symbol ToM = p.getToM().get(0);
		Name propertyName = p.getProperty();
		String value = p.getValue();
		
		Motivator motivator;
		
		if(ToM.equals(Constants.UNIVERSAL_SYMBOL.toString()) || ToM.equals(am.getName()))
		{
			String agent = propertyName.getFirstLiteral().toString();
			if(agent.equals(Constants.SELF_STRING))
			{
				AgentLogger.GetInstance().logAndPrint(p.toString());
				...
				if(_motivators.containsKey(property))
				{
					...
					contributionToNeeds += contributionToNeed;
					...
					if(e != null)
					{
						AgentLogger.GetInstance().logAndPrint("Event " + e.toString() + " triggered drive change");
						if(_appraisals.containsKey(e.toString()))
						{
							float aux = _appraisals.get(e.toString()) + contributionToNeeds;
							AgentLogger.GetInstance().logAndPrint("Composed contribution to drives value: " + aux);
							_appraisals.put(e.toString(), aux);
						}
						else
						{
							AgentLogger.GetInstance().logAndPrint("Single contribution to drives: " + contributionToNeeds);
							_appraisals.put(e.toString(),contributionToNeeds);
						}
					}
					else
					{
						AgentLogger.GetInstance().logAndPrint("Event is null - no information about the event that changed the drive");
						_contributionToNeeds += contributionToNeeds;
					}
				}
			}
		}
	...
	public void parseAdditionalFiles(AgentModel am) throws Exception 
	{
		...
		for (String file : _parsingFiles) {
				SAXparser.parse(new File(file), new NeedsLoaderHandler(am, this));
		}	
	}
	...
	public void validate(AgentModel am) throws Exception
	{
		...
		for(ActionEffectsOnDrives actEff : _actionEffectsOnDrives.values())
		{
			variables = actEff.getActionName().getVariableList();
			variables.add(new Symbol("[AGENT]"));
			
			for(EffectOnDrive effDrive : actEff.getEffects())
			{
				if(!_motivators.containsKey(effDrive.getDriveName()))
				{
					AgentLogger.GetInstance().logAndPrint("ERROR: drive " + effDrive.getDriveName() + " used in action " + actEff.getActionName() + " was not declared in the role file!");
    				throw new Exception("ERROR: drive " + effDrive.getDriveName() + " used in action " + actEff.getActionName() + " was not declared in the role file!");
				}
				
				if(!effDrive.getTarget().isGrounded() && !variables.contains(effDrive.getTarget()))
				{
					AgentLogger.GetInstance().logAndPrint("ERROR: variable " + effDrive.getTarget() + " used in EffectOnDrive in action " + actEff.getActionName() + " needs to be declared in action's name (unless it is [AGENT]).");
    				throw new UnspecifiedVariableException("ERROR: variable " + effDrive.getTarget() + " used in EffectOnDrive in action " + actEff.getActionName() + " needs to be declared in action's name (unless it is [AGENT]).");
				}
				
				if(effDrive.getValue() != null && !effDrive.getValue().isGrounded() && !variables.contains(effDrive.getValue()))
				{
					AgentLogger.GetInstance().logAndPrint("ERROR: variable " + effDrive.getValue() + " used in EffectOnDrive in action " + actEff.getActionName() + " needs to be declared in action's name (unless it is [AGENT]).");
    				throw new UnspecifiedVariableException("ERROR: variable " + effDrive.getValue() + " used in EffectOnDrive in action " + actEff.getActionName() + " needs to be declared in action's name (unless it is [AGENT]).");
				}
			}
		}
		
		for(ExpectedGoalEffectsOnDrives goalEff : _goalEffectsOnDrives.values())
		{
			ActivePursuitGoal g = (ActivePursuitGoal) am.getGoalLibrary().GetGoal(goalEff.getGoalName());
			
			variables = IntegrityValidator.getListOfVariables(g.getPreconditions());
			
			for(EffectOnDrive effDrive : goalEff.getEffects())
			{
				if(!_motivators.containsKey(effDrive.getDriveName()))
				{
					AgentLogger.GetInstance().logAndPrint("ERROR: drive " + effDrive.getDriveName() + " used in goal " + goalEff.getGoalName() + "  was not declared in the role file!");
    				throw new Exception("ERROR: drive " + effDrive.getDriveName() + " used in goal " + goalEff.getGoalName() + "  was not declared in the role file!");
				}
				
				if(!effDrive.getTarget().isGrounded() && !variables.contains(effDrive.getTarget()))
				{
					AgentLogger.GetInstance().logAndPrint("ERROR: variable " + effDrive.getTarget() + " used in EffectOnDrive in Goal " + goalEff.getGoalName() + " must be used in the goal's preconditions!");
    				throw new UnspecifiedVariableException("ERROR: variable " + effDrive.getTarget() + " used in EffectOnDrive in Goal " + goalEff.getGoalName() + " must be used in the goal's preconditions!");
				}
				
				if(effDrive.getValue() != null && !effDrive.getValue().isGrounded() && !variables.contains(effDrive.getValue()))
				{
					AgentLogger.GetInstance().logAndPrint("ERROR: variable " + effDrive.getValue() + " used in EffectOnDrive in Goal " + goalEff.getGoalName() + " must be used in the goal's preconditions!");
    				throw new UnspecifiedVariableException("ERROR: variable " + effDrive.getValue() + " used in EffectOnDrive in Goal " + goalEff.getGoalName() + " must be used in the goal's preconditions!");
				}
			}
		}
	}
