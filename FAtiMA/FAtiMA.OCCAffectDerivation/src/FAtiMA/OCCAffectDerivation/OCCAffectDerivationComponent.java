...
public class OCCAffectDerivationComponent implements Serializable, IAffectDerivationComponent, IModelOfOtherComponent {
	...
	private static OCCBaseEmotion OCCAppraisePraiseworthiness(Event event, float praiseworthiness) {
		...
		if(praiseworthiness >= 0) {
			if(event.GetSubject().equals(Constants.SELF_STRING)) {
	...
	private static OCCBaseEmotion OCCAppraiseCompoundEmotions(Event event, float desirability, float praiseworthiness)
	{
		...
		if(desirability > 0)
		{
			if(praiseworthiness > 0)
			{
				...
				if(event.GetSubject().equals(Constants.SELF_STRING))
				{
					em = new OCCBaseEmotion(OCCEmotionType.GRATIFICATION,potential, event, Name.ParseName(Constants.SELF_STRING));
				}
				else
				{
		...
		else if(desirability < 0)
		{
			if(praiseworthiness < 0)
			{
				...
				if(event.GetSubject().equals(Constants.SELF_STRING))
				{
					em = new OCCBaseEmotion(OCCEmotionType.REMORSE,potential, event, Name.ParseName(Constants.SELF_STRING));
				}
				else
				{
