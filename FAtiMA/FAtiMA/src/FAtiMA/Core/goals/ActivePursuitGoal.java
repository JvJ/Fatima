...
public class ActivePursuitGoal extends Goal implements IPlanningOperator {
	...
	public ActivePursuitGoal(Name description) {
		...
		//IPlanningOperator
		_agent = new Symbol(Constants.SELF_STRING);
	...
	public boolean CheckFailure(AgentModel am) {
		...
		for (Condition c : _failureConditions){
			if(c.getValidSubstitutions(am) != null)
			{
	...
	{
	    val.findUnreachableConditions(am,_name.toString(),_successConditions);
	    
	    ArrayList<Symbol> unboundVariables = IntegrityValidator.getListOfVariables(_preConditions);
	    
	    checkForUnspecifiedVariables(_successConditions,unboundVariables,"SuccessCondition");
	    checkForUnspecifiedVariables(_failureConditions,unboundVariables,"FailureCondition");
	    checkForUnspecifiedVariables(_cancelConditions,unboundVariables,"CancelCondition");
	    
	    unboundVariables.addAll(IntegrityValidator.getListOfVariables(_cancelConditions));
	    unboundVariables.addAll(IntegrityValidator.getListOfVariables(_failureConditions));
	    unboundVariables.addAll(IntegrityValidator.getListOfVariables(_successConditions));
	    
	    HashMap<String,Integer> variableCount = countVariables(unboundVariables);
	    for(Entry<String, Integer> e : variableCount.entrySet())
	    {
	    	if(e.getValue() == 1)
	    	{
	    		AgentLogger.GetInstance().logAndPrint("WARNING: variable " + e.getKey() + " of the goal " + this.getName() + " is used only once in the goal definition. Please make sure this is not a typo.");
	    	}
	    }
	}
	
	private HashMap<String,Integer> countVariables(ArrayList<Symbol> variables)
	{
		Integer oldValue;
		...
		{
			if(variableCount.containsKey(s.toString()))
			{
				oldValue = variableCount.get(s.toString());
				variableCount.put(s.toString(), oldValue+1);
			}
			else
			{
				variableCount.put(s.toString(), 1);
			}
		}
	...
	private void checkForUnspecifiedVariables(ArrayList<Condition> conditions, ArrayList<Symbol> unboundVariables, String conditionType) throws UnspecifiedVariableException
	{
		for(Condition cond : conditions)
	    {
	    	for(Symbol s : cond.getName().getLiteralList())
	    	{
	    		//if the symbol is not grounded, it means it is a variable and
	    		//we need to check if the variable already appeared in the goal's preconditions
	    		if(!s.isGrounded())
	    		{
	    			if(!unboundVariables.contains(s))
	    			{
	    				AgentLogger.GetInstance().logAndPrint("ERROR: "+ conditionType + " " + cond + " of the goal " + this.getName() + " uses a variable not specified in the goal's preconditions!");
	    				throw new UnspecifiedVariableException("ERROR: "+ conditionType + " " + cond + " of the goal " + this.getName() + " uses a variable not specified in the goal's preconditions!");
	    			}
	    		}
	    	}
	    	...
	    	{
	    		for(Symbol s : cond.getValue().getLiteralList())
		    	{
		    		//if the symbol is not grounded, it means it is a variable and
		    		//we need to check if the variable already appeared in the goal's preconditions
		    		if(!s.isGrounded())
	...
	public boolean mayContainSelf()
	{
		...
		while(li.hasNext())
		{
			...
			if(cond.isGrounded())
			{
				if(cond.getName().toString().contains(Constants.SELF_STRING))
				{
					return true;
				}
			}
