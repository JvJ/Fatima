...
public class SemanticMemory implements Serializable {
	...
	{
		return _kb.GetInferenceOperators();
	}
	
	public void SetInferenceOperators(ArrayList<Step> operators)
	{
		_kb.SetInferenceOperators(operators);
	}
	...
	public void InferEffects(Step infOp, AgentModel am)
    {
    	Condition cond;
    	AgentModel perspective;
    	
    	AgentLogger.GetInstance().logAndPrint("@" + am.getName() + "(SELF:" + am.isSelf() + ") inferseffects: " + infOp.toString());
    	
    	for(Effect eff : infOp.getEffects())
    	{
    		if(eff.isGrounded())
    		{
    			cond = eff.GetEffect();
    			perspective = am.getModelToTestCondition(cond);
    			if(perspective != null)
    			{
    				//if the new perspective is different from the current one (used by the AgentModel)
    				...
    				AgentLogger.GetInstance().logAndPrint("adding " + cond.getName() + " = " + cond.getValue() + " to ToM " + perspective.getName() + "(SELF:" + perspective.isSelf() + ")");
					AgentLogger.GetInstance().logAndPrint("ToM " + cond.getToM());
