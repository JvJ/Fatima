...
public class Step implements IPlanningOperator, Cloneable, Serializable {
	...
	protected float _baseprob;
	protected boolean _selfExecutable;
	...
	public Step(Symbol agent, Name action, float probability) {
		...
		_selfExecutable = (!_agent.isGrounded()) || 
				_agent.toString().equals(Constants.SELF_STRING);
	 ...
	 */
	public void CheckIntegrity(AgentModel am, IntegrityValidator val) throws UnspecifiedVariableException {
	    Step s2;
	    Step s3;
	    s2 = (Step) this.clone();
	    ...
	    //val.findUnreachableConditions(am, this._name.toString(),_preconditions);
	    ...
	    if(Unifier.Unify(this._agent, s2._agent, substs) &&
	        ...
	        while(lc1.hasNext()) {
	            ...
	            if(!c2.getName().equals(c3.getName()) || !c2.getValue().equals(c3.getValue()))
	            {
	            	System.out.println("WARNING: variable used in condition " + c1 + " has to be declared in the step's name: " + _name);
	        ...
	        while(le1.hasNext()) {
	            ...
	            if(!c2.getName().equals(c3.getName()) || !c2.getValue().equals(c3.getValue())) 
	            {
	            	System.out.println("WARNING: variable used in effect " + c1 + " has to be declared in the step's name: " + _name);
	                throw new UnspecifiedVariableException(this._name.toString(),c1.toString());
	            }
	        }
	    }
	...
	{
		if(this.getName().getLiteralList().size() > 1){
			//means that the action has a target
			return this.getName().getLiteralList().get(1);
		}else return null;
	}
