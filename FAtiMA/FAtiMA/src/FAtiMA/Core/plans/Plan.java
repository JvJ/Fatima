...
public class Plan implements Cloneable, Serializable
{
	...
	private static IPlannerUnifierStrategy plannerUnifierStrategy = new DefaultPlannerUnifierStrategy();
    ...
    public static void setPlannerUnifierStrategy(IPlannerUnifierStrategy strat)
    {
    	Plan.plannerUnifierStrategy = strat;
    }
    
    public static IPlannerUnifierStrategy getPlannerUnifierStrategy()
    {
    	return Plan.plannerUnifierStrategy;
    }
    ...
    private void CheckProtectedConstraints(IPlanningOperator op)
    {
        ...
        while (li.hasNext())
        {
            ...
            {
                eff = (Effect) li2.next();
                if(Plan.plannerUnifierStrategy.isThreat(eff.GetEffect(), cond))
                {
                ...
                if(cond.isGrounded())
                {
                	if(cond.checkCondition(am)==1)
                	{
                		...
                		if(!CreatesCausalConflicts(startLink))
                		{
                    		...
                    		AgentLogger.GetInstance().log("ContinuousP " + this + ": extending a link for condition " + cond + " to start.");
                		}
                	}
                }
        ...
        if(linksToRemove.size() > 0)
        {
        	for(ListIterator<CausalLink> li = linksToRemove.listIterator(); li.hasNext();)
            {
            	...
            	if(CheckRedundantStep(link.getSource()))
            	{
                    ...
                    AgentLogger.GetInstance().log("Continuous P. " + this + " removing redundant step");
            	}
            }
        	//there is one additional problem, removing a redundant step may 
