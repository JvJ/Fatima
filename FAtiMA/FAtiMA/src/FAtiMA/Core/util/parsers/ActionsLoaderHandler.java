...
public class ActionsLoaderHandler extends ReflectXMLHandler {
	...
	public void Action(Attributes attributes) {
		...
		_currentOperator = new Step(Constants.AGENT_SYMBOL,action,probability);
		...
		{
			addEventEffect(action);
		}
	}



	private void addEventEffect(Name action){
		String event;
		String firstName;
		...
		event = "EVENT(" + Constants.AGENT_STRING + "," + firstName;
		while(li.hasNext()) {
			event = event + "," + li.next();
		}
		event = event + ")";
	...
	public void DialogPair(Attributes attributes){
		Name action;
		String aux;
		float probability;
		action = Name.ParseName(attributes.getValue("name"));
		...
		_currentOperator = new DialogPairStep(Constants.AGENT_SYMBOL,action,probability);
		_actionLibrary.addAction(_currentOperator);

		addEventEffect(action);
	}

	public void InitiationAct(Attributes attributes){
		String name = attributes.getValue("name");
		((DialogPairStep)_currentOperator).setInitiationAct(name);
	}

	public void ReactiveAct(Attributes attributes){
		String name = attributes.getValue("name");
		((DialogPairStep)_currentOperator).setReactiveAct(name);
	}
