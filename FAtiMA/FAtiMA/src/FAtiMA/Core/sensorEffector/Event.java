...
public class Event implements Cloneable, Serializable {
	...
	public Event applyPerspective(String agentName)
	{
		...
		{
			e._subject = Constants.SELF_STRING;
		}
		else
		{
		...
		{
			e._target = Constants.SELF_STRING;
		}
		else
		{
		...
		if(this._parameters != null)
		{
			...
			while(li.hasNext())
			{
				...
				{
					p2 = new Parameter("param",Constants.SELF_STRING);
				}
				else
				{
	...
	public Event RemovePerspective(String agentName)
	{
		...
		if(this._subject.equals(Constants.SELF_STRING))
		{
		...
		if(this._target != null && this._target.equals(Constants.SELF_STRING))
		{
			e._target = agentName;
		}
		else
		...
		if(this._parameters != null)
		{
			...
			{
				p1 = li.next();
				if(p1.GetValue().equals(Constants.SELF_STRING))
				{
	...
	public Event(String subject, String action, String target) {
		...
		_action = action;
		_target = target;
		_type = EventType.ACTION;
		_status = ActionEvent.SUCCESS;
	}
	...
	// To print the event on the tables of the agent display
	public String toTableString() {
	    Parameter p;
	    
	    String subject = "ANY", target = "ANY", action = "ANY";
	    
	    if(_subject != null && !_subject.equals("*")){
	    	subject = _subject;
	    }
	    if(_target != null && !_target.equals("*")){
	    	target = _target;
	    }
	    ...
	    String aux = subject + " " + action + " " + target;
	    ListIterator<Parameter> li = _parameters.listIterator();
	    while(li.hasNext()) {
	        p =  li.next();
	        aux = aux + " "  + p.GetValue();
