...
public class PropertyNotEqual extends PropertyCondition {
     ...
     */
    public PropertyNotEqual(Name name, Name value, String ToM) {
	...
	public ArrayList<SubstitutionSet> getValidSubstitutionsUsingPerspective(AgentModel perspective) {
		...
		if (getName().isGrounded()) {
			//if the name is grounded, both name and value are grounded and we
	...
	public ArrayList<SubstitutionSet> GetValidInequalities(AgentModel am) {
		...
		{
			cond = (PropertyNotEqual) this.clone();
		...
		return cond.getValidInequalitiesUsingPerspective(perspective);
	}
	
	private ArrayList<SubstitutionSet> getValidInequalitiesUsingPerspective(AgentModel perspective)
	{
		SubstitutionSet ss;
		...
		if (getName().isGrounded()) 
		{
			ss = this.getSubstitutionsForValue(perspective);
		...
		for(Substitution s : ss.GetSubstitutions())
		{
			subSet.AddSubstitution(new Inequality(s));
		}
