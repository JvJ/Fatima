...
public class FacialExpressionSimulator {
	...
	public FacialExpressionType updateFacialExpression(AgentModel am){
		...
		String previousFacialExpression = (String) am.getMemory().getSemanticMemory().AskProperty(Name.ParseName(Constants.SELF_STRING + FACIAL_EXP_PROPERTY));
		
		if(previousFacialExpression == null){
			//Set the initial FacialExpression
			am.getMemory().getSemanticMemory().Tell(true,Name.ParseName(Constants.SELF_STRING + FACIAL_EXP_PROPERTY), FacialExpressionType.NEUTRAL.name());
		...
		if(newFacialExpression != FacialExpressionType.valueOf(previousFacialExpression)){	
			am.getMemory().getSemanticMemory().Tell(true,Name.ParseName(Constants.SELF_STRING + FACIAL_EXP_PROPERTY), newFacialExpression.name());			
