...
public class EmotionalPlanner implements Serializable {
	...
	private IActionFilterStrategy _aFStrategy = new DefaultStrategy();
		...
		while (li.hasNext()) {
			...
			if(!op.getName().equals(oldOp.getName())) 
			{
				...
				for(int i=0; i < op.getEffects().size();i++)
				{
					...
					if(Plan.getPlannerUnifierStrategy().unifyConditions(effectCond, cond, substs))
					{
						newPlan = (Plan) p.clone();
						
						if (newStep) 
						{
							IPlanningOperator opAux = (IPlanningOperator) op.clone();
							opAux.makeGround(substs);
							boolean filterResult = _aFStrategy.filterActionFromPlan(am,opAux);
	...
	public Plan ThinkAbout(AgentModel am, DeliberativeComponent dp, Intention intention) {
		...
		while(li.hasNext()) {
			...
			System.out.println("Plan probability: " + p.getProbability(am));
			System.out.println("Plan steps: " + p.getSteps());
			System.out.println("ImportanceOfFailure: " + failureImportance*p.getProbability(am));
			...
			//if(threatIntensity >= hopeIntensity && aux >= failureImportance) {
				...
				intention.RemovePlan(p);
				AgentLogger.GetInstance().log("ACCEPTANCE - Plan Dropped - " + intention.getGoal().getName().toString());
				return null;
			}
			else 
			{
		...
		if (flaw != null) {
			...
			/*newPlan = (Plan) p.clone();
			newPlan.IgnoreConflict(am,flaw);
			if (newPlan.isValid()) {
				intention.AddPlan(newPlan);
		...
		if (openConditions.size() > 0) {
			...
			System.out.println("Step: " + p.getOperator(openPrecond.getStep()));
			...
			else 
			{
				...
				{
					substitutionSets = cond.getValidSubstitutions(am);
				}
