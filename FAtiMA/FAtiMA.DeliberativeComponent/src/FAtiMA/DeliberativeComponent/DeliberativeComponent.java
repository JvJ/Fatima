...
IExpectedUtilityStrategy {
	...
	private static final long PLANNING_TIME = 5000;
	 ...
	 *         exists
	 */
	private ValuedAction getSelectedAction(AgentModel am) {
		...
		//if the action is a dialog pair we return the initiation act
		if(_selectedAction instanceof DialogPairStep){
			DialogPairStep dialogPair = ((DialogPairStep)_selectedAction);
			String target = dialogPair.getTarget().toString();
			DialogManager.initiateDialogPair("SELF",dialogPair.getInitiationAct(),dialogPair.getReactiveAct(),target,Name.ParseName(dialogPair.getKey()));
			Name actionName = Name.ParseName("SpeechAct("+target+","+dialogPair.getInitiationAct()+")");
			return new ValuedAction(DeliberativeComponent.NAME, actionName,null);
		}
		...
		if (!_selectedAction.getAgent().isGrounded()) 
		{
			...
			Substitution sub = new Substitution(_selectedAction.getAgent(),
					new Symbol(Constants.SELF_STRING));
		...
		} else if (!_selectedAction.getAgent().toString().equals(Constants.SELF_STRING)) {
	...
	public void actionSelectedForExecution(AgentModel am, ValuedAction selectedAction) {
		...
		else
		{
			...
			e = new Event(Constants.SELF_STRING, action, target);
			while (li.hasNext()) {
	...
	public ValuedAction actionSelection(AgentModel am) {
		...
		if(_currentIntention != null)
		{
			...
			if(_currentIntention.getPlanningTime() <= PLANNING_TIME && _currentIntention.GetSolutions().size() < 10)
			{
			...
			else
			{
				...
				prepareAction(am);
				return getSelectedAction(am);
			}
			
		}
		else if(DialogManager.isActiveDialogPair() && DialogManager.getReplierName().equals("SELF")){
			//if there is an obligation to respond 
			String initiationAct = DialogManager.getInitiationAct();
			Name actionName;

			ArrayList<Name> possibleResponses = DialogPairStep.getInitiationActPossiblePairs(initiationAct);
			Name chosenResponse = null;

			if(possibleResponses.size() > 0 && DialogManager.getDialogPairName() == null){
				chosenResponse = possibleResponses.get((int)Math.round(Math.random()));//TODO: Select the best action		
				DialogManager.setDialogPairName(chosenResponse); 
				DialogPairStep step = (DialogPairStep) am.getActionLibrary().getAction(chosenResponse.toString());
				DialogManager.setExpectedReactiveAct(step.getReactiveAct());
				actionName = Name.ParseName("SpeechAct("+DialogManager.getInitiatorName()+","+step.getReactiveAct()+")");
			}else{
				actionName = Name.ParseName("SpeechAct("+DialogManager.getInitiatorName()+","+DialogManager.getExpectedReactiveAct()+")");
			}
			_selectedAction =  new Step(new Symbol(am.getName()),actionName,1.0f);
			return new ValuedAction(DeliberativeComponent.NAME, actionName,null);
		}
		return null;
	}
	...
	private void prepareAction(AgentModel am)
	{
		...
		// that the plan was successfully
		// achieved
		...
		if (_actionMonitor == null && _selectedPlan != null) {
			...
			if (copingAction != null) 
			{
				if (!_currentIntention.IsStrongCommitment()) 
				{
					AgentLogger.GetInstance().logAndPrint("Plan Commited: " + _selectedPlan.toString());
	...
	public void setActionFilterStrategy(IActionFilterStrategy strat){
		this._planner.setActionFilterStrategy(strat);
	}
	...
	public void update(AgentModel am, Event event) {
		...
		if (_actionMonitor != null && _actionMonitor.matchEvent(event)) {
			if (_actionMonitor.getStep().getAgent().isGrounded()
					&& !_actionMonitor.getStep().getAgent().toString().equals(Constants.SELF_STRING)) 
			{
	...
	/**
	 * Returns a list with all protection constraints from interest goals
	 * @author Nuno Ferreira
	 * @return All protection constraints
	 */
	public ArrayList<ProtectedCondition> getProtectionConstraints(){
		return _protectionConstraints;
	}
	
	/**
	 * Resets all intentions
	 * @author Nuno Ferreira
	 * @param aM
	 */
	public void resetAllIntentions(AgentModel aM){
		Iterator<Intention> iter = this.getIntentionsIterator();
		while(iter.hasNext()){
			Intention intention = iter.next();
			intention.ResetPlans();
			intention.GetSolutions().clear();
			ActivePursuitGoal g = intention.getGoal();
			Plan newPlan = new Plan(this.getProtectionConstraints(), g.GetSuccessConditions());
			intention.AddPlan(newPlan);
		}
	}
