...
public class Intention implements Serializable {
	...
	private static final int FORGET_TIMER = 2000;
	...
	public Plan getBestSolution(AgentModel am)
	{
		...
		float bestValue;
		float tempH;
		//rank solutions by heuristic function for now
		...
		bestSolution = _solutions.get(0);
		bestValue = bestSolution.h(am);
		
		for(int i=1;i<_solutions.size();i++)
		{
			tempH = _solutions.get(i).h(am);
			if(tempH < bestValue)
			{
				bestSolution = _solutions.get(i);
				bestValue = tempH;
			}
		}
