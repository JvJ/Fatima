...
\begin{figure}[h!]
  ...
  Action Tendencies:
  \[
  \lambda x.(knows(self, mother(x,self) \wedge thin(x)) \mapsto isHappy(self))
  \]
  \[
  \lambda x.(knows(self, mother(x,self) \wedge \neg thin(x)) \mapsto \neg isHappy(self))
  \]
  ...
  Action Tendencies:
  \[
  knows(self, isHandsome(self)) \mapsto isHappy(self)
  \]
  ...
  \caption{An example of goal, action tendency, and action
    ...
    the environment.  Unbiound variables are introduced with $\lambda$-notation.}
  \label{example_goals}
...
unbound variable \emph{x} is introduced, which unifies with
\emph{self}'s mother and being thin or not thin.  The results (after
the $\mapsto$ symbol) are either happiness or uhnappiness.  Thus, the
action tendencies for Good Guy can be described as ``If I know my
mother is thin, I am happy.'' and ``If I know my mother is not thin, I
...
\subsubsection{Example Plan with Action Selection}

\begin{figure}[h!]
  \caption{Bad Guy's plan for insulting Good Guy's mother.}
\end{figure}

\subsubsection{Example Plan with Action Construction}
